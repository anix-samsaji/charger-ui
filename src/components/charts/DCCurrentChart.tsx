...
export default function DCCurrentChart({ data }: Props) {
  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      <ComposedChart data={data} margin={{ top: 6, right: 4, bottom: 4, left: 0 }}>
        ...
        />
        <Tooltip
          formatter={(v: number, name: string) => [`${v} A`, name]}
        ...
        />
        <Area type="monotone" dataKey="dcADemand" name="Demand"
          stroke="rgb(var(--m3-secondary))" strokeWidth={2} fill="url(#gADem)"
          dot={false} activeDot={{ r: 4, strokeWidth: 0 }} isAnimationActive={false} />
        <Area type="monotone" dataKey="dcASupply" name="Supply"
          stroke="rgb(var(--m3-primary))" strokeWidth={2} fill="url(#gASup)"
          dot={false} activeDot={{ r: 4, strokeWidth: 0 }} isAnimationActive={false} />
      </ComposedChart>
    </ResponsiveContainer>
  )
